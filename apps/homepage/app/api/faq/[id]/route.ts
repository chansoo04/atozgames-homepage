...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const url = process.env.LOGIN_AUTH_URL + "cs.FAQService/GetFAQ";
  ...
  if (!response.ok) {
    return NextResponse.json(
      { result: "failure", message: "알 수 없는 이유로 FAQ 정보를 가져올 수 없습니다" },
      { status: 400 },
    );
  }
  ...
  if (!responseData.success) {
    if (responseData.error_message.includes("not found")) {
      ...
      return NextResponse.json(
        { result: "failure", message: "FAQ 정보를 가져올 수 없습니다" },
        { status: 400 },
      );
    }
  }
