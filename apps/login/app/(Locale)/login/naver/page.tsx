...
import { getAccountState, withdrawalRevoke, signIn } from "../action";
import { signInWithNaver } from "./action";
...
export default function Page() {
  ...
  const getCOOKIE = async () => {
    const req = await fetch("/api/cookie/getByProvider", {
      method: "POST",
      body: JSON.stringify({ provider: GpSignProvider.NAVER }),
    });

    if (!req.ok) {
      return alert("네이버 로그인 정보를 가져오는데 실패했습니다");
    }
  ...
  const signHandler = (signRes: any) => {
    ...
    setIsLoading(false);
    if (signRes.success) {
      // logger.debug("signIn res.sucess", signRes);
      return openToast({
        msg: "로그인 성공",
        type: "success",
      });
    } else {
      console.error(signRes.code, "CODE!!!!!!!");
      switch (signRes.code) {
        // * 로그인 상태 Error
        ...
        default: {
          // logger.error('signIn uncaught code', signRes.code);
  ...
  const signInFromCallback = async (token: any) => {
    console.log(token, "token");
    setIsSigning(true);
    const res = await signInWithNaver(token);
    console.log(res, "왜 SUCCESS가 FALSE일까??");
    signHandler(res);
  };
                  ...
                  >
                    <div className="flex w-full items-center justify-center gap-4">
                      <div className="absolute left-5 top-3 flex items-center justify-center">
                        <Image width={20} height={20} src="/naver-icon.svg" alt="네이버 로그인" />
                      </div>
                      <div className="mt-[3px] flex items-center text-left">네이버 로그인</div>
                    </div>
                  </button>
                </div>
              </div>
