...
import { getSign, handleSign } from "../auth.service";
import { GpSign, GP_SIGN_AUTH_CODE } from "common/cookie";

export async function POST(request: NextRequest) {
  console.warn("!!!START signWithCredential API!!!");
  const data = await request.json();
  console.log("ROUTE.TS", "DATA: ", data);

  const url = process.env.GRPC_API_URL + "game.AuthService/AccountSignInWithCredential";
  const req = await fetch(url, {
    ...
    body: JSON.stringify({ credential: JSON.stringify(data.credential) }),
  });
  console.log(req, "REQ");
